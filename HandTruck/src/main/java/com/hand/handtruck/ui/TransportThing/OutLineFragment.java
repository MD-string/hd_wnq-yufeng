...
import com.hand.handtruck.ui.TransportThing.adapter.TransportOutLineAdapter;
...
public class OutLineFragment extends BaseFragment implements OnClickListener, XListView.IXListViewListener {


	private XListView list_normal;
	private Context mcontext;
	private TransportOutLineAdapter madapter;
	...
	private void initView(View view) {
		...
		//		View headView = View.inflate(getActivity(), R.layout.frg_off_power, null);
		//		list_normal.addHeaderView(headView);
		...
		madapter=new TransportOutLineAdapter(mcontext,list);
			...
			switch (msg.what) {
				case ConstantsCode.MSG_REQUEST_SUCCESS:
					...
					if(tlist !=null && tlist.size() >0){
						...
						Intent erIntent=new Intent(ConstantsCode.TRANSPORT_OUTLINE);
						erIntent.putExtra("tranps_total",totalElement);
				...
				case ConstantsCode.MSG_REQUEST_SUCCESS1:
					...
					if(tlist !=null && tlist.size() >0){
						...
						tv_current_page.setText(currentPage+"");
						tv_total_page.setText("/"+t1);

						Intent erIntent=new Intent(ConstantsCode.TRANSPORT_OUTLINE);
						erIntent.putExtra("tranps_total",totalElement1);
						mcontext.sendBroadcast(erIntent);
					}else{
						ToastUtil.getInstance().showCenterMessage(mcontext, "数据为空");
					}
				...
				case ConstantsCode.MSG_REQUEST_SUCCESS2:
					...
					Intent erIntent=new Intent(ConstantsCode.TRANSPORT_OUTLINE);
					erIntent.putExtra("tranps_total",totalElement2);
					mcontext.sendBroadcast(erIntent);
					break;
