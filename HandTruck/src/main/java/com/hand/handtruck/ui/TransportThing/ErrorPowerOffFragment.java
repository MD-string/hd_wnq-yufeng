...
import com.hand.handtruck.ui.TransportThing.adapter.TransportDuanDianAdapter;
import com.hand.handtruck.ui.TransportThing.bean.TransportBean;
import com.hand.handtruck.ui.TransportThing.presenter.ErrorPowerOffTask;
...
public class ErrorPowerOffFragment extends BaseFragment implements OnClickListener, XListView.IXListViewListener {


	private XListView list_error;
	private Context mcontext;
	private TransportDuanDianAdapter madapter;
	private List<TransportBean> list=new ArrayList<>();
	private ErrorPowerOffTask tranTask;
	...
	private void initView(View view) {
		...
		madapter=new TransportDuanDianAdapter(mcontext,list);
		...
		tranTask = ErrorPowerOffTask.getInstance(mcontext, mHandler);
			...
			switch (msg.what) {
				case ConstantsCode.MSG_REQUEST_SUCCESS:
					...
					if(tlist !=null && tlist.size() >0){
						...
						Intent erIntent=new Intent(ConstantsCode.TRANSPORT_ERROR_DUANDIAN);
						erIntent.putExtra("error_poweroff",totalElement);
				...
				case ConstantsCode.MSG_REQUEST_SUCCESS1:
					...
					if(tlist !=null && tlist.size() >0){
						...
						Intent erIntent=new Intent(ConstantsCode.TRANSPORT_ERROR_DUANDIAN);
						erIntent.putExtra("error_poweroff",totalElement1);
						mcontext.sendBroadcast(erIntent);
					}else{
						ToastUtil.getInstance().showCenterMessage(mcontext, "数据为空");
					}
				...
				case ConstantsCode.MSG_REQUEST_SUCCESS2:
					...
					Intent erIntent=new Intent(ConstantsCode.TRANSPORT_ERROR_DUANDIAN);
					erIntent.putExtra("error_poweroff",totalElement2);
					mcontext.sendBroadcast(erIntent);
					break;
