...
public class TransportOutLineAdapter extends BaseAdapter {
    private Context mContext;
    private List<TransportBean> mList;

    public TransportOutLineAdapter(Context mContext, List<TransportBean> list) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            holder = new ViewHold();
            convertView = LayoutInflater.from(context).inflate(R.layout.item_transport_outline, null);
        ...
        String offDate=cb.getOffTime();

        String onDate=cb.getOnTime();

        holder.tv_out_date.setText(offDate+"");
        holder.tv_ouline_address.setText(cb.getAddress()+"");

        holder.tv_end_date.setText(onDate+"");
